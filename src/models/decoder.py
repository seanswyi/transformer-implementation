...
class Decoder(nn.Module):
    ...
    def forward(self, x, enc_x):
        attn_output = self.dropout1(self.masked_multihead_attention(x, x, x))
        output1 = self.layernorm(x + attn_output)

        attn_output2 = self.dropout2(self.multihead_attention(x, enc_x, enc_x))
        output2 = self.layernorm(attn_output2 + output1)

        output3 = self.layernorm(output2 + self.dropout3(self.ffnn(output2)))
